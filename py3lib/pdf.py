...
4. 测试：pdfpath是/config/workspace/llmnote/workspace/过期米老鼠/过期米老鼠.pdf ，输出目录是/config/workspace/llmnote/workspace/frames/，如果没有frames函数自动创建。
"""
        ...
        for page_num, page in enumerate(pdf.pages, start=1):
                    ...
                    for row_idx, row in enumerate(table):
                        if row and len(row) >= 2:
                            ...
                            if filename and isinstance(filename, str):
                                match = re.search(r'(frame\d+\.\d+\.\d+-\d+(?:\.\d+)?\.jpg)', filename)
                                if match:
                                    print(f"    行 {row_idx}: 找到文件名 {match.group(1)}")
                                    img_to_filename[(table_idx, row_idx)] = match.group(1)
            ...
            if not img_to_filename:
                ...
                if text:
                    ...
                    # 查找所有文件名
                    matches = re.findall(r'(frame\d+\.\d+\.\d+-\d+(?:\.\d+)?\.jpg)', text)
                    if matches:
                        print(f"  从页面文本中找到 {len(matches)} 个文件名")
                        for i, filename in enumerate(matches):
                            img_to_filename[(0, i)] = filename
...
if __name__ == "__main__":
    ...
    output_dir = "/config/workspace/llmnote/workspace/frames/"
    os.makedirs(output_dir, exist_ok=True)
